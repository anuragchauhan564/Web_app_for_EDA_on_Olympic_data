...
# line plot for nation participation over years
            nation_over_time = helper.data_over_time(df_summer,'region')
            fig = px.line(nation_over_time, x ='Edition',y = 'region')
            st.title(":blue[Participating Nations over the Years]")
            st.plotly_chart(fig)

# line plot for Events over the year
            events_over_time = helper.data_over_time(df_summer,'Event')
            fig = px.line(events_over_time, x ='Edition',y = 'Event')
            st.title(":blue[Events over the Years]")
            st.plotly_chart(fig)

# line plot for Athletes over the year
            athlete_over_time = helper.data_over_time(df_summer,'Name')
            fig = px.line(athlete_over_time, x ='Edition',y = 'Name')
            st.title(":blue[Athletes over the Years]")
            st.plotly_chart(fig)

# heatmap for no. of events over thr time
            st.title(":blue[No. of Events over time(Every Sport)]")
...
# table for most successful Athletes Overall or sports wise
            st.title(":blue[Most Successful Athletes]")
            ...
            st.table(x.style.background_gradient(cmap="Reds"))
...
# top 10 athletes for selected country how won medals numbers of time 
        st.title(":blue[Top 10 Athetes of]  " + selected_country)
        top10_df = helper.most_successful_countrywise(df_summer,selected_country)
        st.table(top10_df.style.background_gradient(cmap="Reds"))
...
# Scatterplot for winning medal over hight vs weight
        ...
        fig,ax = plt.subplots()
        ax = sns.scatterplot(temp_df['Weight'],temp_df['Height'],hue=temp_df['Medal'],style=temp_df['Sex'],s=80)
